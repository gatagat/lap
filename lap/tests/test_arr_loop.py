...
def prepare_sparse_cost(shape, cc, ii, jj, cost_limit):
    ...
    ii_ = np.r_[ii, np.arange(0, n, dtype=np.uint32), np.arange(n, n + m, dtype=np.uint32), n + jj]
    jj_ = np.r_[jj, np.arange(m, n + m, dtype=np.uint32), np.arange(0, m, dtype=np.uint32), m + ii]
...
def test_lapjv_arr_loop():
    ...
    cc = np.array(
        [
            ...
            1.706431415909629434e-01,
            2.192929371231896185e-01,
            2.117769622802734286e-01,
            2.604267578125001315e-01,
        ]
    )
